...
fn test_parse_fixture_file() {
    ...
    let has_updating = cargo_check_step.lines.iter()
        .any(|line| line.display_content.contains("Updating"));
    let has_downloading = cargo_check_step.lines.iter()
        .any(|line| line.display_content.contains("Downloading"));
    let has_error = cargo_check_step.lines.iter()
        .any(|line| line.display_content.contains("error"));

    assert!(has_updating, "Should contain 'Updating crates.io index' output");
    ...
    assert_eq!(cargo_check_step.name, "Run cargo check",
        "Step name should be 'Run cargo check' without ##[group] prefix");

    // Verify ##[group] lines are marked as metadata and hidden
    let has_group_line = cargo_check_step.lines.iter()
        .any(|line| line.display_content.contains("##[group]"));
    assert!(!has_group_line, "##[group] lines should be marked as metadata and not appear in step lines");

    // Verify [command] prefix is removed from command lines
    let git_version_step = job_node.steps.iter()
        .find(|s| s.name.contains("Git version"))
        .expect("Should find 'Getting Git version info' step");

    let has_command_prefix = git_version_step.lines.iter()
        .any(|line| line.display_content.starts_with("[command]"));
    assert!(!has_command_prefix, "[command] prefix should be removed from display_content");

    // Verify command lines are marked as is_command
    let has_command_lines = git_version_step.lines.iter()
        .any(|line| line.is_command);
    assert!(has_command_lines, "Lines with [command] prefix should be marked as is_command=true");
